...
function evaluatePromptResult(
  ...
  if (isCompanyFieldTablePrompt(prompt.prompt)) {
    ...
    if (/待核验|未确认|待确认|待核实|未核实|公开文件未单列|公开披露未细分|缺数据|未获取|未取得|缺乏|无法确认|N\/A|待发|待财报更新|未单独披露|需以官方公告为准|待官方验证|待官方公告|未经其他来源交叉确认|精确份额需|无独立公开|本次搜索摘要未直接列出|请参阅/.test(parsed.answer)) issues.push("vague company field placeholder");
