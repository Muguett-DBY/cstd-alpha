...
- **估值实验室**：A 股经营型公司自动生成五年量化估值基线，支持手动覆写、情景敏感性、不可变版本与预测复盘；银行、保险和周期股保留专用估值卡
...
- 已加入研究队列的 A 股经营型公司可创建可编辑估值；系统根据年度财务与最新行情自动预填未来五年 FCFF DCF 基线及终值参数。
- 手动修改会标记为用户锁定值，并优先于自动基线；每次保存都会创建新版本，历史版本不会被覆盖。
- 浏览器预览和服务端保存使用同一套确定性公式。AI 仅用于解释与辅助，不直接决定最终计算结果。
- 数据快照、来源引用、缺失项 fallback 和警告会随版本保留；配置 `TUSHARE_TOKEN` 可增强 A 股结构化数据，但不会隐藏数据缺口。
- 银行、保险和周期股继续使用剩余收益或中周期专用估值卡，当前不进入五年 DCF 编辑器。
