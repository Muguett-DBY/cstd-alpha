...
function buildSubjectOnlyClarificationRequest(message: string): AssistantChoiceRequest | null {
  ...
  if (!containsLikelyResearchSubject(message)) return null;
  // 如果消息包含投资/分析相关的关键词（不只是公司名），说明用户已有明确意图，不需要澄清
  if (/(股价|多少|走势|估值|PE|PB|ROE|EPS|市值|分析|对比|表现|技术|指标|行情|今天|最新|新闻|公告|财报|利润|营收|增长|分红|融资|资金|流入|流出|热点|题材|板块|行业|排名|龙虎榜|解禁|K线|RSI|MACD|均线|趋势|怎么样|怎么看|好不好|可以吗|风险|收益|前景|空间|怎么|如何|什么)/.test(normalized)) return null;
