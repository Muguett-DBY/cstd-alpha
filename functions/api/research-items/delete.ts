...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!body?.ids?.length) return json({ error: "缺少删除目标。" }, 400);
  if (body.ids.length > 50) return json({ error: "单次最多删除 50 项。" }, 400);
  await deleteResearchItems(env.REPORT_LIBRARY_DB, session.userId, body.ids);
